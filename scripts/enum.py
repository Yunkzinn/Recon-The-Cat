...
allSubs = f"{domain.split('.')[0]}_allSubs.txt"
...
os.system(f"cat {allSubs} | httpx -status-code -mc 200 -silent -no-color | tr -d '[]' | anew {httpx200}; cat {httpx200} | awk '{{print $1}}' >> {awkHttpx}; rm -rf {httpx200}; mv {awkHttpx} {httpx200}")
