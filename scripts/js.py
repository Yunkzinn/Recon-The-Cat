...
os.system(f"cat {getJS} {katana} {gau} {waybackurls1} {waybackurls2} {gospider} {hakrawler1} {hakrawler2} {dirsearch} | sort -u | tee {allJS}")
os.system(f"cat {getJS} {katana} {gau} {waybackurls1} {waybackurls2} {gospider} {hakrawler1} {hakrawler2} {dirsearch} | grep -E '\.js(?:onp?)?$' | sort -u | tee {justJS}")
...
#Add In Future:
#os.system(f"cat dominios | gau | grep -iE '\.js'| grep -iEv '(\.jsp|\.json)' >> gauJS.txt ; cat dominios | waybackurls | grep -iE '\.js'| grep -iEv '(\.jsp|\.json)' >> waybJS.txt ; gospider -a -S dominios -d 2 | grep -Eo '(http|https)://[^/\"].*\.js+' | sed 's#\] \- #\n#g' >> gospiderJS.txt ; cat gauJS.txt waybJS.txt gospiderJS.txt | sort -u | tee >> saidaJS ; rm -rf *.txt ; cat saidaJS | anti-burl |awk '{{print $4}}' | sort -u | tee >> AliveJs.txt ; xargs -a AliveJs.txt -n 2 -I@ bash -c 'echo -e '\n[URL]: @\n'; python3 linkfinder.py -i @ -o cli' ; cat AliveJs.txt  | python3 collector.py output ; rush -i output/urls.txt 'python3 SecretFinder.py -i {{}} -o cli | sort -u | tee >> output/resultJSPASS'")